...
public class CartPageController extends AbstractPageController
{
	...
	/**
	 * Perform price override and show success or error messages afterwards. The price override is performed after form
	 * validation in the cartFacade with a dedicated method.
	 */
	@RequestMapping(value = "/priceUpdate", method = RequestMethod.POST)
	public String updateTotalPrice(@RequestParam("entryNumber") final long entryNumber, @RequestParam("reasonNum") final int reasonNum,
			//@RequestParam("pORCPk") final long pORCPk, @RequestParam("productCode") final String productCode,
			final Model model,
			 @Valid final UpdatePriceForm form, final BindingResult bindingResult, final HttpServletRequest request,
			final RedirectAttributes redirectModel)
	{
		if (!bindingResult.hasErrors())
		{
			try{
				//TODO: need some validation here if price override was successful
				PriceOverrideModel priceOverride=cartFacade.updateCartEntryTotalPrice(entryNumber, form.getTotalPrice(),
						Integer.valueOf(reasonNum), form.getReasonText());
				
				GlobalMessages.addFlashMessage(redirectModel, GlobalMessages.CONF_MESSAGES_HOLDER, "Price override performed");
			}
			catch (final CommerceCartModificationException ex)
			{
				LOG.error("Couldn't update total price of product with the entry number: " + entryNumber + ".", ex);
			}
		}
		else
		{
			for (final ObjectError error : bindingResult.getAllErrors())
			{
				if (error.getCode().equals("typeMismatch"))
				{
					GlobalMessages.addErrorMessage(model, "Mismatch of types");
					GlobalMessages.addFlashMessage(redirectModel, GlobalMessages.ERROR_MESSAGES_HOLDER, "Mismatch of types");
				}
				else
				{
	 ...
	 * and the list of price override reasons.
	 */
	private void performTelesalesActions(Model model)
	{		
		final String AS_MODE_SESSION_ATTRIBUTE =  "de.hybris.platform.assistedservicefacades.AssistedServiceFacade_as_mode";
		boolean isAsmShown=(sessionService.getCurrentSession().getAttribute(AS_MODE_SESSION_ATTRIBUTE) !=null 
				?	((Boolean) sessionService.getCurrentSession().getAttribute(AS_MODE_SESSION_ATTRIBUTE)).booleanValue()			
				: false);
		model.addAttribute("isAsmShown", Boolean.valueOf(isAsmShown));
